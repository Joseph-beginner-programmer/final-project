...
1. **Write `RejectPurchaseOrderAction`** (`PendingApproval → Rejected`) — near-mirror of the now-complete `ApprovePurchaseOrderAction`, minus the `approved_by`/`approved_at` writes. Unblocked (Gate + enum transition both already fixed).
